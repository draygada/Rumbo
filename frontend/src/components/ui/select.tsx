...
interface SelectTriggerProps extends Omit<HTMLAttributes<HTMLButtonElement>, "children">, VariantProps<typeof triggerVariants> {
  ...
  placeholder?: string;
  error?: string;
  /** When false, only the dropdown chevron is shown (e.g. next to a custom input). Default true. */
  showLabel?: boolean;
}

const SelectTrigger = forwardRef<HTMLButtonElement, SelectTriggerProps>(
  ({ className, variant, icon: Icon, placeholder = "Select…", error, showLabel = true, "aria-label": ariaLabel, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        <button
          ...
          aria-label={!showLabel ? (ariaLabel ?? placeholder) : ariaLabel}
          disabled={disabled}
          ...
          className={cn(
            ...
            !showLabel && "min-w-0 w-11 shrink-0 justify-center gap-0 px-0",
            className,
          )}
          {...props}
        >
...
const SelectContent = forwardRef<HTMLDivElement, SelectContentProps>(
  ({ className, children, highlightStyle = "sliding" }, ref) => {
    ...
    const { portalStyle, menuMaxHeight, transformOrigin, motionY } = useMemo(() => {
      const gap = 6;
      const pad = 12;
      const menuCap = 300;
      ...
      const rawMax = openUpward ? spaceAbove - gap : spaceBelow - gap;
      const nextMenuMaxHeight = Math.max(96, Math.min(menuCap, rawMax));
      const left = Math.max(pad, Math.min(triggerRect.left, vw - pad - triggerRect.width));
      ...
      <SelectContentContext.Provider value={{ registerItem, activeIndex, checkedIndex, highlightStyle }}>
        <div style={portalStyle}>
          <motion.div
            ...
            className={cn(
              `relative flex flex-col gap-0.5 overflow-y-auto ${shape.container} bg-card shadow-[0_4px_12px_rgba(0,0,0,0.02)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-border/60 p-1 select-none outline-none`,
              className,
            )}
            initial={{ opacity: 0, y: motionY, scaleY: 0.96 }}
